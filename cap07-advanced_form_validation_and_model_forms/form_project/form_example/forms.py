...
def validate_email_domain(value):
    if value.split('@')[-1].lower() != 'example.com':
        raise ValidationError('The email address must been on the domain example.com.')
...
class OrderForm(forms.Form):
    ...
    def clean_email(self):
        ...
        item_total = cleaned_data.get('magazine_count', 0) \
    ...
    def clean(self):
        ...
        if cleaned_data['send_confirmation'] and not cleaned_data.get('email'):
            self.add_error('email', \
                           'Please enter an email address to receive the \
                           confirmation message.')
        elif cleaned_data.get('emai') and not cleaned_data['send_confirmation']:
            self.add_error('send_confirmation', \
                           'Please check this if you want to receive a \
                           confirmation email.')
